...
const Calendar = () => {
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        ...
        try {
            ...
            setAllQuotes(prev => prev.map(q =>
                q.id === orderId ? { ...q, status: newStatus } : q
            ))
            if (selectedEvent && selectedEvent.id === orderId) {
    ...
    const events = useMemo(() => {
        return allQuotes.map((q, i) => {
            ...
            const statusStyle = getStatusStyle(q.status || 'New')
                                    ...
                                    <div className="space-y-1">
                                        {dayEvents.slice(0, 3).map(event => (
                                            <button
                                                ...
                                                style={{ backgroundColor: event.statusStyle.bg, color: event.statusStyle.text }}
                                                className="w-full text-left px-2 py-1 rounded-md text-[11px] font-semibold truncate hover:opacity-90 transition-opacity"
                                            >
                                                {event.shortTitle}
                                            </button>
                                        ))}
                                    ...
                                    <div className="space-y-2">
                                        {dayEvents.map(event => (
                                            <button
                                                ...
                                                onClick={() => setSelectedEvent(event)}
                                                style={{ backgroundColor: event.statusStyle.bg, color: event.statusStyle.text }}
                                                className="w-full text-left px-2 py-2 rounded-lg text-[11px] font-semibold hover:opacity-90 transition-opacity"
                                            >
                        ...
                        <div className="divide-y divide-slate-100">
                            {dayEvents.length === 0 ? (
                                ...
                                dayEvents.map(event => (
                                    ...
                                    >
                                        <div className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: event.statusStyle.bg }} />
                                        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setSelectedEvent(event)}>
                                            <p className="text-sm font-semibold text-slate-900">{event.title}</p>
                                            <p className="text-xs text-slate-400">{event.email} &middot; {event.items} items</p>
                                        </div>
                                        <WorkflowDropdown
                                            currentStatus={event.status}
                                            onStatusChange={(newStatus) => updateOrderStatus(event.id, newStatus)}
            ...
            {selectedEvent && (
                <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setSelectedEvent(null)}>
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="px-6 py-4" style={{ backgroundColor: getStatusStyle(selectedEvent.status).bg, color: getStatusStyle(selectedEvent.status).text }}>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <WorkflowDropdown
                                            ...
                                            onStatusChange={(newStatus) => updateOrderStatus(selectedEvent.id, newStatus)}
