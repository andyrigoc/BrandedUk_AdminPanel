    ...
    const parseQuoteData = (order) => {
        try {
            ...
            if (data?.customizations?.length > 0) {
                console.warn('ORDER', order.quote_id, 'FULL CUSTOMIZATIONS:', JSON.stringify(data.customizations))
            }
            if (data?.logos && Object.keys(data.logos).length > 0) {
                console.warn('ORDER', order.quote_id, 'LOGOS OBJECT:', JSON.stringify(data.logos))
            }
            return {
                basket: data?.basket || [],
